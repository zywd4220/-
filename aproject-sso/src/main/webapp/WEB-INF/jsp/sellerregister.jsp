...
<div class="w" id="logo">
    <div>
    	<a href="http://localhost:8082">
    		<img src="/images/meituan-logo.png" alt="美团商城" width="170" height="60"/>
    	</a> <b></b>
    </div>
</div>
...
<div class="w" id="regist">
    <div class="mt">
        ...
        <div class="extra">
        <span>我已经注册，现在就&nbsp;
        	<a href="/page/sellerlogin" class="flk13">登录</a>
        </span>
        </div>
    </div>
    <div class="mc">
        <form id="personRegForm" method="post" onsubmit="return false;">
            <div class="form" onselectstart="return false;">
                <div class="item" id="select-regName">
                    <span class="label"><b class="ftx04">*</b>商家名：</span>

                    <div class="fl item-ifo">
                        <div class="o-intelligent-regName">
                            <input type="text" id="regName" name="sellername" class="text" tabindex="1" autoComplete="off"
                ...
                <div class="item item-new">
                    ...
                    <div class="fl item-ifo">
                        ...
                        <label for="protocol">我已阅读并同意<a href="#" class="blue" id="protocol">《美团用户注册协议》</a></label>
                        <span class="clr"></span>
...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/seller/check/"+escape($("#regName").val())+"/1?r=" + Math.random(),
	            	success : function(data) {
	            		if (data.data) {
	            			//检查手机号是否存在
	            			$.ajax({
	            				url : REGISTER.param.surl + "/seller/check/"+$("#phone").val()+"/2?r=" + Math.random(),
				            	success : function(data) {
				            		if (data.data) {
					            		REGISTER.doSubmit();
				            		} else {
		...
		doSubmit:function() {
			$.post("/seller/register",$("#personRegForm").serialize(), function(data){
				if(data.status == 200){
					alert('用户注册成功，请登录！');
					REGISTER.login();
				} else {
		...
		login:function() {
			 location.href = "/page/sellerlogin";
			 return false;
		},
		reg:function() {
			if (this.inputcheck()) {
